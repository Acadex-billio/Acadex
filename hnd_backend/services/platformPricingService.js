...
const DEFAULTS = {
  ...
  materials: {
    report: { basic_preview_pages: 1, paygo_preview_pages: 3, paygo_full_preview_price: 0, paygo_download_price: 0, paygo_access_minutes: 60 },
    presentation: { basic_preview_pages: 1, paygo_preview_pages: 3, paygo_full_preview_price: 0, paygo_download_price: 0, paygo_access_minutes: 60 },
    question_paper: { basic_preview_pages: 1, paygo_preview_pages: 3, paygo_full_preview_price: 0, paygo_download_price: 0, paygo_access_minutes: 60 },
  },
...
function buildCenterPricing(doc) {
  ...
  ['create', 'join'].forEach((action) => {
    ['basic', 'pro', 'paygo'].forEach((plan) => {
      const entry = center?.[action]?.[plan] || {};
      result[action][plan] = {
        amount: toNumber(entry.amount, DEFAULTS.center[action][plan].amount),
        currency: String(entry.currency || DEFAULTS.center[action][plan].currency),
